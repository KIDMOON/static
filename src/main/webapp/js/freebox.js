...
$().ready(function (){
	...
	var params = $.getArgs();
	// 免费类型
	if (!$.isBlank(params["type"])) {
		freeType = params["type"];
	}
	tabChange(freeType);
	// 加载数据
	getFreeInfo();
	...
	$("#open-box-div-left a").click(function () {
		...
		// 显示价格
		$("#boxPriceSpan").text("$ " + (boxPrice * _num).toString().formatMoney());
	...
	// 打开箱子动作
	$("#open").click(function () {
		if ($("#open").attr("data-status") == "1" || boxStatus == 0 || boxStatus == 2) {
			return;
		}
		$("#open").text("打开中...");
		...
		$.gdAjax({
			url : "index/freeboxopen/" + boxId + "/" + freeType,
			type : "GET",
...
function tabChange(index) {
	$(".free_cases_nav>a").removeClass("active");
	$(".page_content>.free_cases").hide();
	$(".free_cases_nav>a").eq(index).addClass("active");
	$(".page_content>.free_cases").eq(index).show();
	$("#free-title").text($(".free_cases_nav .active").find("div").text());
}
...
function getFreeInfo() {
	$.gdAjax({
		url : "free/everydaylist?type=" + freeType,
		type : "GET",
		success : function(data) {
			// 加载免费信息
			initFreeInfo(data.result);
			// 加载箱子信息
			initBoxInfo(data.result);
		}
	});
}
 ...
 * 加载免费信息
 */
function initFreeInfo(result) {
	var userFreeBox = result.userFreeBox;
	if (userFreeBox != null) {
		var leftCount = parseInt(userFreeBox.allCount - userFreeBox.openedCount);
		if (leftCount > 0) {
			$("#open").text("免费开启" + leftCount + "次");
		}
	}
	if (freeType == 0) {
		$("#free-desc").text("每邀请一人取回成功即可免费抽奖一次");
	}
	else if (freeType == 1) {
		$("#free-desc").text("每邀请新人用户首充即可免费抽奖一次，多邀多得");
	}
	else if (freeType == 2) {
		$("#free-desc").text("首次成功充值$" + result.rechargeMoney + "即可得到一次免费箱子开箱机会");
	}
}